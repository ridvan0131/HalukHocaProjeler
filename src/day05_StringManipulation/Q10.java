package day05_StringManipulation;

import java.util.Scanner;

public class Q10 {
    public static void main(String[] args) {
            /*

         * Kullanıcıdan aldığınız  Adı, Soyadı ve kredi kartı numaralarını aşağıdaki gibi özel forma dönüştürün
         * Kredi kartı numarasını kontrol edin, yoksa 16 haneden küçük olursa   "Geçersiz kredi kartı numarası" yazdırın.
         * input : John White 1234234534561478
           output: Ad : J*** Soyad :W**** kartNo:***********1478 (Adın baş harfleri ve soyadı büyük olmalıdır)
         */

		 /* Regular Expression(regex) : Belli turdeki characterleri secebilmemize yarar.
		 1) \\d    ==> tum rakamlar         0->9
		  	 \\D    ==> tum rakam disi character ler  0->9    _ disindaki hersey
		 2) \\w   ==> A->Z		a->z
		 	 \\W  ==> A->Z		a->z		tum harf disi character ler
		 3) \\s   ==> space
		 	 \\S   ==> space disindaki hersey
	 */
        Scanner scanner=new Scanner(System.in);
        System.out.print("Lutfen isminizi giriniz: ");
        String isim=scanner.nextLine();
        System.out.print("Lutfen soyadinizi giriniz: ");
        String soyad=scanner.nextLine();
        System.out.print("Lutfen kredikarti numaranizi aralarinda bosluk olmadan 16 karakter olarak giriniz: ");
        String kkno=scanner.nextLine();

        if (kkno.length()==16){
            System.out.println("Isim: "+isim.toUpperCase().charAt(0)+isim.substring(1).replaceAll("\\w","*")+"\n"+
                                "Soyad: "+soyad.toUpperCase().charAt(0)+soyad.substring(1).replaceAll("\\w","*")+"\n"+
                                "Kart Numarasi: "+kkno.substring(0,12).replaceAll("\\d","*")+kkno.substring(12));
        }else {
            System.out.println("Kredi kartinizi aralarinda bosluk olmadan 16 karakter olarak giriniz...");
        }
    }
}
